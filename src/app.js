/**
 * SMABBLE
 * Control your smappee energy monitor from your pebble smartwatch
 *  GNU GENERAL PUBLIC LICENSE v3
 * by Serge Besnard - 2016
 */
var MENU_LOCATION = 0;
...
var update_location_menu = function() {
    ...
    if (typeof servLoc != 'undefined') {
        ...
        for (var i = 0; i < servLoc.length; i++) {
            menu.item(MENU_LOCATION, i, {
                title: servLoc[i].name,
                subtitle: 'ID = ' + servLoc[i].locationID
            });
        }
    } else {
        console.log("WARNING : no serviceLocation defined");
        menu.items(MENU_LOCATION, [{
            title: 'not defined'
        }]);
    }
};
...
var display_locations_ok = function(result) {
    if (typeof result.error == 'undefined') {
        ...
        for (var i = 0; i < result.length; i++) {
            ...
            Settings.data("smappee_conf").serviceLocations[i] = {
                'name': result[i].name,
                'locationID': result[i].serviceLocationId
            };
